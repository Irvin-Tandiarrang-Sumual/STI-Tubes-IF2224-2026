...
private:
    ...
    // General helpers
    void addError(const CSTNodes* node, const std::string& message);
    bool isNonTerminal(const CSTNodes* node, NonTerminal nt) const;
    bool isTerminal(const CSTNodes* node, TokenType type) const;
    std::string tokenText(const CSTNodes* node) const;
    std::string lowered(std::string text) const;
    std::string operatorText(TokenType type) const;

    const CSTNodes* childAt(const CSTNodes* node, std::size_t index) const;
    const CSTNodes* firstChildOf(const CSTNodes* node, NonTerminal nt) const;
    const CSTNodes* firstTokenOf(const CSTNodes* node, TokenType type) const;

    template <typename T>
    std::unique_ptr<T> withLocation(std::unique_ptr<T> node, const CSTNodes* source) const {
        if (node && source) node->location_ = source->getLocation();
        return node;
    }
